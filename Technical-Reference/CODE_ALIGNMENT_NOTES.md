...
## 16. External ecosystem adapters — current state and kOA migration target

The supplied Orgo snapshot does **not** contain the Interaction Kernel Konnaxion adapter or the IK/Da’at Kristal v5 adapter documented by the current kOA target. The generic bridge is implemented and remains the compatibility boundary.

Do not rewrite this implementation status as if IK were already present. Migration should preserve the existing Orgo reliability machinery rather than add a second outbox/lifecycle.
...
Target Interaction Kernel profiles:

- inbound: `governance.decision.execute/1.0.0`;
- outbound: `accountability.impact.publish/1.0.0`.

Reuse Signal, `IntegrationOperation`, Outbox, idempotency and terminal receipt semantics. Never map Case↔Topic or Task↔Consultation by identity.

### Kristal / Da’at

New v5 work should target Da’at through explicit IK Profiles such as `kristal.build.request/1.0.0` and `kristal.revision.request/1.0.0`. The current direct `validate` bridge is compatibility-only until that adapter is implemented.

Orgo approval is not Kristal validation/recognition. A required receipt predicate may block an Orgo process without implying a Kristal-wide “no compile on fail” rule.
...
Treat host trust/resources/lifecycle and physical Runtime Pack activation as platform concerns. With kOA-Linux, kOA-Linux owns verify/stage/activate/rollback state; Orgo may only track/request work when a real operational Task/Case is required.
