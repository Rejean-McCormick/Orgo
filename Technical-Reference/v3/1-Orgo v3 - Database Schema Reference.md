...
> **Historical baseline note (2026-09-16):** This v3 document records the earlier schema baseline. `apps/api/prisma/schema.prisma`, applied migrations and `../IMPLEMENTATION_STATUS.md` are the current authority. Section 14 describes additions that were missing from the historical snapshot; it must not be read as proof that those models remain absent today.
