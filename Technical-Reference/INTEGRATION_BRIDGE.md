...
> **2026-09-16 ecosystem alignment:** This is the **implemented generic/legacy compatibility bridge** for the current Orgo snapshot. The kOA target architecture uses Interaction Kernel Profiles for Konnaxion and Da’at/Kristal v5. Do not interpret this bridge as native provider-schema conformance. See `INTERACTION_KERNEL.md`.
...
| Adapter | Operations |
| --- | --- |
| Kristal | `validate` (legacy/generic compatibility operation; not a Kristal v5 compile contract) |
...
`DurableProcess` waits for these receipts with an explicit deadline. To require a business predicate, declare `expect` on the integration step. Paths are relative to receipt `data`, for example `{"expect":{"validated":true}}`. A successful transport without that required value blocks the **Orgo process**. This predicate does not define Kristal v5 compilation or Reference eligibility globally. A completed workflow's `ACTIONS_COMMITTED` and a completed process still never mutate Work status by implication. Unsupported compensation operations are rejected by the same adapter allowlist.
