...
> **Ecosystem alignment (2026-09-16):** The current kOA cross-system target uses Interaction Kernel (IK) Profiles for Konnaxion↔Orgo and Da’at for new Kristal v5 workflows. The supplied Orgo snapshot still implements the generic bridge, so IK is a migration target here rather than a current capability. A required Kristal result may gate an Orgo process, but does not create a universal Kristal validation-before-compile rule.
