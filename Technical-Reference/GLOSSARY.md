...
Signal is persisted as a first-class Orgo object before retry-prone orchestration. A Signal is not automatically a Task or Case.
...
The target distributed cross-system protocol for the kOA ecosystem. In this supplied Orgo snapshot it is a migration target, not an implemented adapter. IK does not own Orgo domain state or replace Orgo's outbox.

## Working Exchange / Reference Exchange

Kristal v5 concepts. A Working Exchange can exist before final validation/recognition when policy permits. A Reference Exchange is recognized for declared authority/scope. Orgo must keep these epistemic states separate from Task/Case status.

## Da’at

The kOA anti-corruption/mapping boundary for new IK-facing Kristal v5 workflows. Orgo targets Da’at rather than assuming a native Kristal endpoint.
