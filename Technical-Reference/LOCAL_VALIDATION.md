...
## Interaction Kernel migration acceptance

The current snapshot does not claim IK implementation. When the migration is introduced, add deployment/integration evidence for:

- `governance.decision.execute/1.0.0` admission and finalized DecisionRecord handling;
- same idempotency key + same semantic fingerprint replay;
- same key + different fingerprint conflict;
- one accepted decision producing one deduplicated Orgo Signal/work consequence;
- `accountability.impact.publish/1.0.0` through existing `IntegrationOperation`/Outbox and terminal reconciliation;
- Da’at `kristal.build.request/1.0.0` and `kristal.revision.request/1.0.0`;
- exact Kristal v5 pin and Working/Reference status preservation;
- kOA Build Record v2 / Release Record v2 compatibility;
- no Orgo-owned competing physical Runtime Pack activation state.

Until those tests exist and pass, the generic bridge remains the only implemented external-integration contract claimed by this snapshot.
